...
{	
  ...
  if (! GLOBAL_INIT_PARAMETER_MODEL_FILE_NAME.empty()){// ## qcheng75 -->
    ...
    cout << "Found Init Parameter Model File" << endl;
    print();
    cout<<"\n"<<endl;
  }
  else{    // ## qcheng75 -->
...
double BasePredictor::test( const vector< vector< Sequence > >& testSeqs, const vector< vector< double > >& testBindingData, int perfOption ) const
{
  ...
  // Spearman correlation between predictions and observations
  if ( perfOption == 1 ) {
    vector< double > corrs; 
    for ( int i = 0; i < nExps; i++ ) {
      printf("rank correlation code being called\n");
      corrs.push_back( rank_correlation( predicted[ i ], testBindingData[ i ] ) );
    }
...
{	
  ...
  cout <<"Initial model "<< obj_model << endl;
  ...
  for ( int i = 0; i < nRandStarts; i++ ) {
    ...
    if ( obj_model > obj_best ) {
      par_best = par_model;
      obj_best = obj_model;
      cout <<"Find better model "<< obj_best << endl;		
    }
  }	
  ...
  cout<<"final model "<<obj_model<<endl;	
...
{	
  ...
  for ( int i = 0; i < seqs.size(); i++ ) {
    ...
    if (BindingPredictor::perfOption == 0) 
      corrs.push_back( correlation( predicted, bindingData[ i ] ) );
    else if (BindingPredictor::perfOption == 1)
      corrs.push_back( rank_correlation( predicted, bindingData[ i ] ) );
    else {
      printf("perfOption not recognized\n"); exit(1);
    }
    predicted.clear();//qcheng75
  }	
...
{
  cout << "Start simplex_minimize minimization" << endl;
  int nFactors = nInFactors() + nOutFactors();
...
{
  cout << "Start minimization gradient_minimize_KTconstrained" << endl;
  int nFactors = nInFactors() + nOutFactors();
...
void BindingCrossValidator::runThermoModel( vector< double >& results, const vector< Motif >& motifs, const vector< double >& energyThrs, const FactorIntFunc* intFunc, bool featureSelection, int modelOption )
{
  ...
  for ( int k = 0; k < nFolds; k++ ) {
    ...
    // testing	
    double result = predictor->test( testSeqs[ k ], testBindingData[ k ], BindingPredictor::perfOption );
...
int BindingPredictor::perfOption = 0;
