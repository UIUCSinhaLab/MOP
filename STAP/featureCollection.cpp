...
int main( int argc, char* argv[] ) 
{
	...
	string paraModelFile, featureStatiscsTrFile, featureStatiscsTeFile; // ## qcheng75 parameter model will be saved to the file
	int nrandstarts = 1; //## SS
	int fixcooperror = 0, chiprescaling=0, isFeatureStat=0; //## qcheng75
	for ( int i = 1; i < argc; ++i ) {
		if ( !strcmp( "-m", argv[ i ] ) )
		  ...
		  nrandstarts = atoi(argv[++i]);        // ## SS
			...
			fixcooperror = atoi(argv[++i]);        // ## qcheng75  correct the cooperativity range and parameter setting
		...
		else if ( !strcmp( "-fstr", argv[ i ] ) ){ // ## qcheng75 feature statistics f(seq,motif)=<bound sites, LLRenergy
			featureStatiscsTrFile = argv[++i];
			isFeatureStat = 1;
		}
		else if ( !strcmp( "-fste", argv[ i ] ) ){ // ## qcheng75 feature statistics f(seq,motif)=<bound sites, LLRenergy
...
// 		cv.print();
	...
	if ( !paraModelFile.empty() ) {
		...
		predictor->getPar().print2Stream(pmodel);
		pmodel.close();
	}
