...
double correlation( const vector< double >& v1, const vector< double >& v2 )
{
	...
	double* working = new double[ 2*n ];
	for ( int i = 0; i < n; i++ ) {
		data1[ i ] = v1[ i ];
		data2[ i ] = v2[ i ];	
	}
	
	double ret = gsl_stats_correlation( data1, 1, data2, 1, n );
	delete data1;
	delete data2;
	delete working;
	return ret;
...
double rank_correlation( const vector< double >& v1, const vector< double >& v2 )
{
	assert( v1.size() == v2.size() );
	int n = v1.size();
	...
	double ret = gsl_stats_spearman( data1, 1, data2, 1, n, working );
