...
int main( int argc, char* argv[] ) 
{
  ...
  double effectThr = 0.5;        // feature effect threshold, related to featureSelection
  int K = 0;		// K-fold cross-validation, best to do this outside. 
  int modelOption = 0; 	// model option: 0 - basic model; 1 - stepwise model
  int perfOption = 0;		// option for measuring performance: 0 - Pearson correlation; 1 - Spearman (rank) correlation
  ...
  BindingPredictor::perfOption = 0; // SS
  ...
  BindingPredictor::perfOption = perfOption;
  ...
  //## qcheng75 
  // SS: ignore this piece of code, we will not use feature statistics options
  ...
  // cross validation
  // SS: ignore this piece of code; we will do cross validation outside of STAP
  if ( K > 0 ) {
  ...
  if (areAllPGiven){ // this indicates that parameter values are provided through commandline
    ...
    if (isFreePGiven){ // this indicates that the binding weight parameter is known to be = fFreePMaxBinding for every TF
  ...
  if ( !testSeqFile.empty() ) {
    ...
    //## qcheng75
    // SS: we dont care about this piece of code.
    ...
    // testing
    double testResult;
    testResult = predictor->test( testSeqs, testBindingData, BindingPredictor::perfOption );	
